...
For each missing page, the workflow uses structured model output to create five transcript-grounded critique sections plus a two-part overall assessment: a dark epistemic-reality rebuke and a dark red challenge section aimed at the weakest points in the transcript. It verifies direct quotes against the stored transcript, resolves research anchors only from `research/source-index.json`, retries drafts that fail a quality check, and renders each episode page atomically before any non-API site-wide refresh runs.

The generation step deliberately runs with `--skip-site-refresh`, then uploads a `critique-generation-recovery-<run-id>` artifact containing the generated episode pages, ignored draft JSON files, and a small manifest. This is a recovery guardrail: if navigation, homepage, SEO, or tests fail after API generation, the generated critique output can be inspected or restored without repeating the same model calls. After that checkpoint, the workflow refreshes adjacent-episode navigation and the five most recent homepage cards, rebuilds SEO discovery files, runs the full test suite, and commits only validated `docs/` changes.

The workflow reuses the existing `OPENAI_API_KEY` secret. `CRITIQUE_MODEL` is optional and defaults to `gpt-5.5`; `CRITIQUE_REASONING_EFFORT` defaults to `high` in the batch command. A manual dispatch can override the model or limit the number of episodes, while the scheduled run processes all missing critiques. GitHub schedules are fixed UTC; the ingest workflow runs at `14:15 UTC`, which is 9:15 EST and 10:15 EDT.
...
4. Fill every `TODO` in `critique-draft.json`. Each substantive section must include:
   ...
   - moral non-realist ethical/normative phrasing that treats ethical language as human discourse rather than stance-independent norm-furniture;
...
Critique depth is a production contract, not a copy-editing preference. The validator rejects draft specs or rendered pages that omit the five-card method framework (`Calibration`, `Symmetry`, `Architecture`, `Alternatives`, and `Bounded Agency`), section-level Free of Faith anchors, underdeveloped `◉` explanations, shallow research notes, one-row audit tables, thin tag applications, under-explained formalizations, short evidence-calibration tests, underdeveloped epistemic-reality rebukes, missing dark red challenge sections, reified ethical language, repeated boilerplate, label-like AI steelman claims, deprecated quote-strip summary cards, public links to the private OnReason source index, or lowercase proper names inherited from ASR/transcript text.

The standing epistemic stance is that epistemic faith is intrinsically irrational: confidence should be proportioned to evidence, and any appeal to faith as a basis for belief is a rational defect rather than an epistemic virtue. Biblical faith-language can express trust, reliance, or loyalty as relational commitment, but it must not be treated as evidence for truth or a warrant for belief merely because it uses the word “faith.” Critiques should steelman faith claims charitably without rehabilitating faith-based confidence as rational.

The standing ethical stance is moral non-realism. Critiques may analyze ethical language, harms, welfare, preferences, emotions, social coordination, and public justification, but they should not write as if ethical discourse names stance-independent norm-furniture or an ontological object.
